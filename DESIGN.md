...
**The One Source Rule.** The palette lives in `panel_kit_core::tokens` and
nowhere else. Every other place a colour appears is either derived from it at
build time or pinned to it by a test:

| Copy | How it stays true |
|---|---|
| `assets/panel-kit.css` `:root` | hand-written, pinned by a test that parses each declaration and compares it to the token |
| `panel-kit-tui`'s `Theme::DARK` | derived from the token constants through `const fn rgb(Token)` |
